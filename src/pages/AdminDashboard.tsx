...
export default function AdminDashboard() {
  ...
  const [collegeSortOrder, setCollegeSortOrder] = useState<'high-to-low' | 'low-to-high' | 'alphabetical'>('high-to-low');
  const [collegePage, setCollegePage] = useState(1);
  const [collegePerPage, setCollegePerPage] = useState(10);

  const [domainSearchQuery, setDomainSearchQuery] = useState('');
  const [domainSortOrder, setDomainSortOrder] = useState<'high-to-low' | 'low-to-high' | 'alphabetical'>('high-to-low');
  const [domainPage, setDomainPage] = useState(1);
  const [domainPerPage, setDomainPerPage] = useState(10);
  ...
  const domainCountsMap = users.reduce<Record<string, number>>((acc, u) => {
    const dom = u.internshipDomain?.trim() || 'Unassigned Track';
    acc[dom] = (acc[dom] || 0) + 1;
    return acc;
  }, {});
  ...
  const getTabValue = (t: string | null) => {
    ...
    if (t === 'assignment' || t === 'assignments' || t === 'student-reports') return 'student-reports';
    if (t === 'test-report' || t === 'test-reports') return 'test-reports';
    if (t === 'internship-report' || t === 'reports') return 'reports';
    if (t === 'cyber-cafe' || t === 'emitras' || t === 'cyber-cafe-summary') return 'cyber-cafe-summary';
    if (t === 'college-wise') return 'college-wise';
    if (t === 'domain-wise') return 'domain-wise';
    if (t === 'teachers') return 'teachers';
    if (t === 'sub-users') return 'sub-users';
    if (t === 'notifications') return 'notifications';
    if (t === 'college-export') return 'college-export';
    return t;
  };

  const initialTab = getTabValue(new URLSearchParams(location.search).get('tab'));
  const [activeAdminTab, setActiveAdminTab] = useState(initialTab);

  useEffect(() => {
    const tabFromUrl = getTabValue(new URLSearchParams(location.search).get('tab'));
    if (tabFromUrl) {
      setActiveAdminTab(tabFromUrl);
    }
  }, [location.search]);
  ...
  const handleTabChange = (value: string) => {
    const targetTab = getTabValue(value);
    if (isSubUser && targetTab !== 'dashboard') {
      setActiveAdminTab('dashboard');
      return;
    }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto p-8">
        ...
        >
          ...
          <TabsContent value="student-reports" className="space-y-8 mt-4">
            <div className="space-y-6">
              <div className="student-card p-6 bg-white/80">
          ...
          <TabsContent value="test-reports" className="space-y-8 mt-4">
            <div className="space-y-6">
              {/* Summary Stats */}
              {(() => {
      ...
      {/* 8. TEST REPORT VIEW */}
          ...
          <TabsContent value="reports" className="space-y-8 mt-4">
            <div className="student-card p-6 bg-white/80">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                  <div className="student-icon bg-indigo-50 text-indigo-600 ring-indigo-100">
